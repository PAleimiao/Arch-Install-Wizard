...
### 无需 Python，纯 Bash + Dialog

Arch Live CD 默认没有 Python 和 tkinter，本向导使用 **Bash + dialog** 编写，
Live CD 中只需一行命令安装 dialog 即可运行：
...
| 机制 | 说明 |
|---|---|
| [SCAN] 双系统检测 | 扫描 EFI/NTFS/ext，发现即停 |
| [BEEP] 蜂鸣器警报 | 发现双系统响 3 声 |
| [LOCK] 密码确认 | 必须输入两次，不一致拒绝 |
| [DISK] 格式化确认 | 二次确认 + 配置摘要 |
| [VIEW] 脚本预览 | 生成后不直接执行，先审查 |
